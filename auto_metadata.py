...
def infer_metadata(nav, fund, moneydj=False, sample=False):
    ...
    risk=single('risk_level') or single('風險報酬等級') or '未確認'
    if not re.fullmatch(r'RR[1-5]',risk): risk='未確認'
    reason='MoneyDJ 原始淨值，未加入現金配息' if moneydj else '依上傳欄位'
    return {'基金':fund,'級別幣別':currency if currency in CURRENCIES else '請確認','報酬口徑':basis,'風險報酬等級':risk,'判讀依據':reason}
